...
const OrderDetail: FC = () => {
  ...
  useEffect(() => {
    ...
    // Set up an interval to check for order status updates every 5 seconds
    const intervalId = setInterval(async () => {
      const user = await getCurrentUser();
      if (user && orderId) {
        const userOrders = getOrders(user.id);
        const foundOrder = userOrders.find((o) => o.id === orderId);
        if (foundOrder && order && foundOrder.status !== order.status) {
          setOrder(foundOrder);

          // Also update the product details if status changed
          const allProducts = getAllProducts();
          const productsWithDetails = foundOrder.items.map((item: any) => {
            const productDetails =
              allProducts.find((p) => p.id === item.id) || {};
